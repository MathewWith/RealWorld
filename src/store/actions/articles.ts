...
export const getArticles = () => {
    return async (dispatch: any) => {
        try{
            ...
            dispatch({type: ArticlesActionTypes.GET_ARTICLES, payload: response.data.articles})
        }catch (error) {
            console.log(error)
        }
...
export const getDefaultTags = () => {
    return async (dispatch: any) => {
        try{
         ...
         dispatch({type: ArticlesActionTypes.GET_DEFAULT_TAGS, payload: response.data.tags})
        }catch (error) {
            console.log(error)
        }
...
export const getFavoriteArticles = (userName: any) => {
    return async (dispatch: any) => {
        try{
            const response = await axios.get(`https://api.realworld.io/api/articles?favorited=${userName}&limit=20&offset=0`)
        dispatch({
